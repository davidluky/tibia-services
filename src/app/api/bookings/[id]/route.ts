...
import { isValidTC, priceConfirmationUpdate } from '@/lib/utils'
import {
  sendBookingAccepted,
  sendBookingDeclined,
...
export async function PATCH(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  switch (action) {
    ...
    case 'set_price':
      ...
      // Proposing a price auto-confirms the proposer's side. A genuinely new
      // price clears the counterparty's confirmation; re-proposing the same
      // price preserves it (the DB trigger forbids unsetting a confirmation, so
      // resetting it unconditionally would surface as a 500).
      update = {
        agreed_price_tc: price_tc,
        ...priceConfirmationUpdate({
          proposedPrice: price_tc,
          currentPrice: booking.agreed_price_tc,
          isCustomer,
          isServiceiro,
          customerConfirmed: booking.price_confirmed_by_customer,
          serviceiroConfirmed: booking.price_confirmed_by_serviceiro,
        }),
      }
      break
