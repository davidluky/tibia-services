...
// ─── Booking price negotiation ─────────────────────────────────────────────────
// Computes the confirmation flags when a party proposes a price via `set_price`.
// Proposing auto-confirms the proposer's side. A genuinely new price clears the
// counterparty's confirmation; re-proposing the same price preserves it so the
// DB contract trigger (which forbids unsetting a confirmation) is not violated.
export function priceConfirmationUpdate(opts: {
  proposedPrice: number
  currentPrice: number | null
  isCustomer: boolean
  isServiceiro: boolean
  customerConfirmed: boolean
  serviceiroConfirmed: boolean
}): { price_confirmed_by_customer: boolean; price_confirmed_by_serviceiro: boolean } {
  const priceChanged = opts.currentPrice !== opts.proposedPrice
  return {
    price_confirmed_by_customer: opts.isCustomer
      ? true
      : priceChanged
        ? false
        : opts.customerConfirmed,
    price_confirmed_by_serviceiro: opts.isServiceiro
      ? true
      : priceChanged
        ? false
        : opts.serviceiroConfirmed,
  }
}
