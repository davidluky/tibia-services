...
The browse page currently sorts by `is_registered` then `avg_rating`. Featured listings sit above all non-featured serviceiros, sorted among themselves by `avg_rating` then `created_at`. Cost is 25 TC/day, with duration determined by the amount sent.
...
5. Browse page: featured (active + expires\_at > NOW()) pinned to top, sorted by rating then date; visually highlighted in ServiceiroCard

**Expiry:** Query-time only — no cron. "Is featured" = `status = 'active' AND expires_at > NOW()`. "Timed out pending" = `status = 'pending' AND requested_at < NOW() - interval '24 hours'` (detected client-side; DB status remains `pending`).
...
export interface FeaturedListing {
  ...
  tc_amount: number
  days_requested: number  // GENERATED by DB: tc_amount / 25 — read-only, never set manually
  status: 'pending' | 'active' | 'canceled'
  requested_at: string    // Required for client-side timeout detection (>24h = timed out)
...
```typescript
// Populated by getAllServiceiros() from a joined featured_listings query.
// ISO string if the serviceiro has an active, non-expired featured listing; null otherwise.
...
Returns the serviceiro's most recent non-canceled listing (if any). The `requested_at` field is critical — the client uses it to detect whether a pending listing has timed out (>24h elapsed).
...
Auth required. Serviceiro cancels their own pending listing. Only works on `pending` status — returns 409 for active or already-canceled listings. Clients should treat 404 and 409 as success (listing already removed or inactive) when called from the timed-out UI state.

Steps:
1. Fetch listing via admin client — 404 if not found
2. Verify `listing.serviceiro_id === user.id` — 403 if not
3. Verify `listing.status === 'pending'` — 409 if not
...
3. Compute `expires_at = new Date(Date.now() + listing.days_requested * 24 * 60 * 60 * 1000).toISOString()`
4. Update: `{ status: 'active', confirmed_at: new Date().toISOString(), expires_at }`
5. Return `{ success: true }`
...
### `src/app/browse/page.tsx` (wherever `getAllServiceiros` is called)

After fetching all serviceiros, fetch currently active featured listings using the admin client (to bypass RLS):
...
const featuredMap = new Map(featuredRows?.map(f => [f.serviceiro_id, f.expires_at]) ?? [])
```

When building each serviceiro result object, add:
```typescript
featured_until: featuredMap.get(sp.id) ?? null
```

**Sort order (applied after enrichment):**
```typescript
// 1. Featured first: by avg_rating DESC, then created_at DESC (tiebreaker)
// 2. Non-featured: by is_registered DESC, then avg_rating DESC, then created_at DESC
const featured = results
  .filter(s => s.featured_until && new Date(s.featured_until) > new Date())
  .sort((a, b) => (b.avg_rating ?? 0) - (a.avg_rating ?? 0)
    || new Date(b.profile.created_at).getTime() - new Date(a.profile.created_at).getTime())

const nonFeatured = results
  .filter(s => !s.featured_until || new Date(s.featured_until) <= new Date())
  .sort((a, b) => {
...
Pass `isFeatured` prop to each `ServiceiroCard`:
```tsx
<ServiceiroCard
  key={s.id}
  serviceiro={s}
  isFeatured={!!s.featured_until && new Date(s.featured_until) > new Date()}
/>
```
...
When `isFeatured`:
- Render `⭐ Destacado` badge at the very top of the card (above display name), styled: `text-xs text-gold font-semibold`
- Apply stronger border/glow: `border-gold shadow-gold/20 shadow-md` (replaces the existing `is_registered` conditional border when featured)
...
2. **No listing** (listing is null):
   ...
   - "Solicitar Destaque" button → POST /api/featured → on success, refetch GET and update state
   - Show error inline on failure

3. **Pending within 24h** (`listing.status === 'pending'` AND `Date.now() - new Date(listing.requested_at).getTime() < 24 * 60 * 60 * 1000`):
   - Show: *"Aguardando confirmação do pagamento."*
   - Show: *"Envie exatamente {listing.tc_amount} TC para Cursos Senai em tibia.com."*
   - Show countdown: hours remaining until 24h timeout (computed from `listing.requested_at`)
   - Cancel button → DELETE /api/featured/{listing.id} → on success or 409, reset state to no-listing

4. **Timed out** (`listing.status === 'pending'` AND >24h elapsed — detected client-side):
   - Show: *"Pagamento não confirmado em 24h. Pedido cancelado automaticamente."*
   - "Solicitar novo destaque" button → DELETE /api/featured/{listing.id} (treat 404 and 409 as success) → reset state to no-listing

5. **Active** (`listing.status === 'active'` AND `expires_at > NOW()`):
   - Show: *"✓ Seu perfil está em destaque"*
   - Show: *"Válido até [formatted expires_at]. ([X] dias restantes)"*
...
```typescript
// Pending listings (including timed-out ones), oldest first
...
- **"Pagamentos pendentes"** section: list of pending listings. For each:
  ...
  - If timed out (requested_at > 24h ago): greyed-out row, label *"Expirado — não pago"* (no confirm button)
...
- **"Destaques ativos"** section: list of active listings. For each:
  - Serviceiro display_name, tc_amount, days_requested, "expira em [expires_at date]"
...
Add nav link after existing links:
```tsx
